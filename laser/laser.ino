...
void loop() {
  ...
  for (pos = 0; pos <= random(110,120); pos += random(-2,3)) { // goes from 0 degrees to 180 degrees
    // in steps of 1 degree
    myservox.write(pos);              // tell servo to go to position in variable 'pos'
    myservoy.write(pos);  
    delay(random(15,100));                       // waits 15 ms for the servo to reach the position
  }
  for (pos = 180; pos >= random(90,80); pos -= random(2,-3)) { // goes from 180 degrees to 0 degrees
    myservox.write(pos);              // tell servo to go to position in variable 'pos'
    myservoy.write(pos); 
    delay(random(15,100));                       // waits 15 ms for the servo to reach the position
  }
